...
function rightRotate(nodo) {
  ...
  if (nodo.hijoIzquierdo != null) {
    nodo.hijoIzquierdo.parent = nodo;
  }
  r.hijoDerecho = nodo;
  r.hijoDerecho.parent = r;
  r.parent = null;
  nodo.altura =
    Math.max(
...
function leftRotate(nodo) {
  ...
  if (nodo.hijoDerecho != null) {
    nodo.hijoDerecho.parent = nodo;
  }
  res.hijoIzquierdo = nodo;
  res.hijoIzquierdo.parent = res;
  res.parent = null;
  nodo.altura =
    Math.max(
      obtenerAltura(nodo.hijoIzquierdo),
...
function RBTreeRemove(nodo, valor) {
  ...
  if (valor < nodo.valor) {
    ...
    if (nodo.hijoIzquierdo == null && nodo.hijoDerecho == null) {
      // CASO #1
      fixRBNode(nodo);
      if (nodo === nodo.parent.hijoIzquierdo) {
        nodo.parent.hijoIzquierdo = null;
      } else {
        nodo.parent.hijoDerecho = null;
      }
      nodo.parent = null;
    } else if (nodo.hijoIzquierdo == null || nodo.hijoDerecho == null) {
      // CASO #2
...
function fixRBNode2(nodo) {
  ...
  let grand = nodo.parent.parent;
  let brother;
  if (nodo.color === NEGRO) {
    if (nodo.valor < parent.valor) {
      brother = parent.hijoDerecho;
      if (isRed(brother)) {
        brother.color = NEGRO;
...
function fixRBNode(nodo) {
  while (nodo.parent != null && nodo.color === NEGRO) {
    let parent = nodo.parent;
...
function linkParent(nodo, parent) {
  if (parent) {
    if (nodo.valor < parent.valor) {
      parent.hijoIzquierdo = nodo;
    } else {
      parent.hijoDerecho = nodo;
    }
    nodo.parent = parent;
  }
}
